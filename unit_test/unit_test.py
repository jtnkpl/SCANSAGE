...
class UnitTest:
    def __init__(self, 
        ...
        scansage_init = os.path.join(scansage_module, "__init__.py")
        if not os.path.isfile(scansage_init):
            raise FileNotFoundError("Invalid scansage_module. The directory should contain __init__.py.")
        
        spec = importlib.util.spec_from_file_location("ScanSage", scansage_init)
        scansage = importlib.util.module_from_spec(spec)
        sys.modules["ScanSage"] = scansage
        sys.modules["easyocr"] = scansage
        spec.loader.exec_module(scansage)
        
        self.scansage = scansage
        self.easyocr = scansage
        if not hasattr(self.scansage, 'utils'):
            setattr(self.scansage, 'utils', importlib.import_module('ScanSage.utils'))
        if not hasattr(self.scansage, 'detection'):
            setattr(self.scansage, 'detection', importlib.import_module('ScanSage.detection'))
        if not hasattr(self.scansage, 'recognition'):
            setattr(self.scansage, 'recognition', importlib.import_module('ScanSage.recognition'))
        
        self.scansage_dir = os.path.dirname(scansage.__file__)
        
        print("Unit test is set for ScanSage at {}".format(os.path.abspath(self.scansage_dir)))
    ...
    def set_data(self, test_data):
        ...
        english_mini_bgr, english_mini_gray = self.scansage.utils.reformat_input(images['english']['mini'])
        english_small_bgr, english_small_gray = self.scansage.utils.reformat_input(images['english']['small'])
        ...
        setattr(self.inputs, 'scansage_config', self.dict2attr(solution_book['inputs']['easyocr_config']))
    ...
    def easyocr_read_as(self, image, language):
        return self.scansage_read_as(image, language)

    def scansage_read_as(self, image, language):
        if not isinstance(language, list):
            language = [language]
        reader =  self.scansage.Reader(language)
        _, pred, confidence = reader.readtext(image)[0]
        reader = None
        torch.cuda.empty_cache()
        return pred, confidence
    
    def set_scansage(self):
        ocr = self.scansage.Reader([self.inputs.scansage_config.main_language])
        setattr(self.scansage, 'ocr', ocr)
    ...
    def do_test(self, verbose = None):
        ...
        num_module_pass = 0
        print("Testing ScanSage: {:d} modules will be tested.\n".format(num_module_to_test))
